 ...
 * (explicit, no silent skip). A `literal` source is returned verbatim,
 * untouched (no trimming), without consulting `env` at all. The returned map
 * is credential-env-key → value and is routed to the gateway only, never into
 * the sandbox env. */
