...
export function scaffoldManifest(opts: ScaffoldManifestOpts): string {
  ...
  return [
    ...
    "#",
    "# Supported keys: harness, mounts, args, env. Anything else is rejected by",
    "# `openlock validate`. The workdir mount (target /sandbox/repo) is what",
    "# the agent works in.",
    "",
    `# Agent harness this project runs (set by 'openlock init'). 'openlock sandbox'`,
    "# reads it back so you don't have to re-pass --harness each time.",
